...
class TwitchDB {
  ...
  savePendingVerification(discordId, twitchId, twitchLogin, code) {
    this.db.prepare(`INSERT INTO pending_verifications (discord_id,twitch_id,twitch_login,code,created_at) VALUES (?,?,?,?,?)
      ON CONFLICT(discord_id) DO UPDATE SET twitch_id=excluded.twitch_id, twitch_login=excluded.twitch_login, code=excluded.code, created_at=excluded.created_at`)
      .run(discordId, twitchId, twitchLogin, code, Date.now());
  }
  getPendingVerification(discordId) {
    return this.db.prepare('SELECT * FROM pending_verifications WHERE discord_id=?').get(discordId) ?? null;
  }
